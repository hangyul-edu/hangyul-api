...
- 자동 승급은 가산형(이벤트 기반: 퀴즈 풀이, 문장 완료 등에서 평가됨). 시간 경과로 인한 자동 강등은 없고, 설정 화면에서 수동으로만 내릴 수 있다.
...
- 시즌은 **1주일** 단위 — 월요일 00:00 ~ 일요일 21:00.
- `season_id`는 ISO 주차 라벨을 사용(예: `2026-W17`).
...
- 레벨 수동 변경은 `PATCH /me/learning/{track_id}`로 위임된다. 이후 이벤트부터 새 레벨을 기준으로 자동 승급이 이어진다.
...
current_level N ──설정 화면에서 수동 변경──▶ current_level M (1..max 중 임의)
```

자동 강등은 없으며, 수동 변경으로만 내릴 수 있다.
...
| **Season (시즌)** | 주간 주기(월 00:00 → 일 21:00). 종료 시 승급 / 유지 / 강등 처리 후 포인트 초기화. |
