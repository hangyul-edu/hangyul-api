...
| 목표 | 지원 기능 |
|---|---|
| 자신 있게 일상 한국어 말하기 | 회화 트랙 — 레벨 기반 또는 프롬프트 기반 문장 추천(예: "식당에서 주문할 때 쓰는 문장"), 한글AI 대화, 오디오 재생 |
| 내 수준에서 TOPIK 연습하고 오답에서 배우기 | TOPIK 트랙 — 레벨 기반 또는 프롬프트 기반 문제; 오답 시 AI 챗봇이 자동으로 해설 |
...
| **프롬프트 활용자 Paul** | 상황별 목적이 뚜렷한 중급 학습자. | `POST /recommendations/sentences` / `/questions`에 자유 프롬프트 전달("면접에서 쓸 문장", "피동 문법 문제") → 마음에 드는 항목 북마크 |
